...
const KNOWN_TOKEN_RULES = [
  ...
  { token: '糢糊', expected: '模糊', category: '繁簡 / 異體差異', confidence: '低', recommendation: '待人工確認，不建議直接批次硬改', note: 'CNBible CUV 保留「糢糊」，eBible / Wikisource / CNBible CUVMP 作「模糊」；屬來源不一致或字形政策項目。' },
...
const FIRST_PHASE_LOW_CONFIDENCE_DECISIONS = [
  ['哥林多前書', '哥林多前書 13:12', '糢糊不清', '模糊不清', 'eBible / Wikisource / CNBible CUVMP 作「模糊」，CNBible CUV 保留「糢糊」。', '待人工確認', '來源不一致，屬字形政策；本輪不硬改。'],
];
...
- 檢查範圍：整本聖經 66 卷。
- 本報告最初為只讀盤點；已於第一階段新增高 / 中信心比對決策與修正結果。
- 修正範圍：僅修正第一階段確認應修的內建聖經文字；未改資料庫、抓取經文程式、UI 或正式使用者資料。
...
6. 本輪不把不同譯本用詞、標點、小字註記或異體字直接當錯字；高信心項目仍需使用者確認後才進入修正輪。
...
- 原低信心：${FIRST_PHASE_LOW_CONFIDENCE_DECISIONS.length} 筆，修正 0 筆，不修正 0 筆，待人工確認 ${FIRST_PHASE_LOW_CONFIDENCE_DECISIONS.length} 筆。
...
- \`糢糊 / 模糊\`：eBible、Wikisource、CNBible CUVMP 作「模糊」，CNBible CUV 保留「糢糊」；來源不一致，列待人工確認。
- 節號差異：屬公開來源的節號保留 / 省略 / 分節差異；本輪不修，後續需另開「章節結構政策」任務。

## 不建議本輪修正項目

- \`神 / 上帝\`：屬對照來源版本差異。
- \`裡 / 裏\`、\`著 / 着\`、\`為 / 爲\`、\`麼 / 嗎\`：多半屬字形或版本差異。
...
- 聚焦 \`糢糊 -> 模糊\` 這類來源不一致或字形政策項目。
- \`妝飾\` 目前判定不修；若未來採 CUVMP 現代字形政策，再另案處理。
- 另開 patch 版本與 changelog。

### 階段 3：註記、標點、異體字暫緩

- \`糢糊 / 模糊\`、括號註記、標點、異體字建議待使用者指定字形政策後再處理。
- 若不影響閱讀，不建議大規模改動整本聖經。
