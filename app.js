...
const DEFAULT_BIBLE_ASSET_VERSION = '2026.06.07-1cor-text-v1.1.5';
...
const els = {
  ...
  prayerCreateButton: document.getElementById('prayer-create-button'),
  prayerCloudNotice: document.getElementById('prayer-cloud-notice'),
  prayerFormPanel: document.getElementById('prayer-form-panel'),
  prayerForm: document.getElementById('prayer-form'),
  prayerFormTitle: document.getElementById('prayer-form-title'),
  prayerId: document.getElementById('prayer-id'),
  prayerTitleInput: document.getElementById('prayer-title-input'),
  prayerRequestInput: document.getElementById('prayer-request-input'),
  prayerStatusSelect: document.getElementById('prayer-status-select'),
  prayerRelatedNoteSelect: document.getElementById('prayer-related-note-select'),
  prayerReviewInput: document.getElementById('prayer-review-input'),
  prayerGraceReflectionInput: document.getElementById('prayer-grace-reflection-input'),
  ...
  noteReaderSearchScope: document.getElementById('note-reader-search-scope'),
  ...
  noteReaderSearchBtn: document.getElementById('note-reader-search-btn'),
  noteReaderList: document.getElementById('note-reader-list'),
  noteReaderWriteBtn: document.getElementById('note-reader-write-btn'),
  noteReaderSearchModal: document.getElementById('note-reader-search-modal'),
  noteReaderSearchBackdrop: document.getElementById('note-reader-search-backdrop'),
  noteReaderSearchModalClose: document.getElementById('note-reader-search-modal-close'),
  noteReaderSearchModalCloseBottom: document.getElementById('note-reader-search-modal-close-bottom'),
  noteReaderSearchModalClear: document.getElementById('note-reader-search-modal-clear'),
  noteReaderSearchResults: document.getElementById('note-reader-search-results'),
  noteReaderReadingModal: document.getElementById('note-reader-reading-modal'),
  noteReaderReadingBackdrop: document.getElementById('note-reader-reading-backdrop'),
  noteReaderReadingContent: document.getElementById('note-reader-reading-content'),
  noteReaderReadingClose: document.getElementById('note-reader-reading-close'),
  noteReaderReadingEdit: document.getElementById('note-reader-reading-edit'),
  ...
  authInlineForm: document.getElementById('auth-inline-form'),
...
function isPrayerCloudReady() {
  return !!(state.supabase && state.currentUser && !state.passwordRecoveryActive);
}
...
function getPrayerStatusLabel(status = '') {
  return PRAYER_STATUS_LABELS[normalizePrayerStatus(status)] || PRAYER_STATUS_LABELS[PRAYER_STATUS_ACTIVE];
}
...
function normalizePrayerRecord(record = {}, fallbackUserId = getUserId()) {
  ...
  return {
    ...
    last_reviewed_at: record.last_reviewed_at || record.lastReviewedAt || '',
    created_at: record.created_at || record.createdAt || now,
    updated_at: updatedAt,
  };
}

function buildPrayerPersistencePayload(record = {}, userId = getUserId()) {
  const normalized = normalizePrayerRecord(record, userId);
  const payload = {
    user_id: userId,
    title: normalized.title,
    prayer_request: normalized.prayer_request,
    status: normalized.status,
    related_note_id: normalized.related_note_id || null,
    review_text: normalized.review_text,
    grace_reflection: normalized.grace_reflection,
...
function sortPrayerRecords(records = []) {
  return [...records].sort((a, b) => new Date(b.updated_at || b.created_at || 0) - new Date(a.updated_at || a.created_at || 0));
}

function getPrayerRelatedNote(prayer = {}) {
      ...
      <section class="panel manual-panel">
        ...
        <article class="manual-article">
          ...
          <section id="manual-writing-note" class="manual-section" data-testid="manual-writing-note-section">
            ...
            <ol>
              <li>先在「我的札記」中輸入文字。</li>
              <li>選取想調整的文字，或把游標放在想加入內容的位置。</li>
              <li>點選上方小工具，例如「小標題」「粗體」「引用」或顏色按鈕。</li>
              <li>有選取文字時，網站會把樣式套用到你選取的內容；沒有選取文字時，多數工具會在游標位置放入範例文字，方便你直接改寫。</li>
              <li>「分隔線」會插入在游標附近，用來分開前後段落。</li>
              <li>你會在編輯區看到網站自動加上的小記號，這是為了記住樣式；在預覽與閱讀時會變成好讀的樣式。</li>
              <li>完成後可以點「預覽文章」，或先儲存草稿，確認顯示效果是否符合預期。</li>
            </ol>
            <h3>每個小工具適合怎麼用</h3>
            <ul>
              <li>小標題：用來標出段落標題，例如「經文觀察」「我的反思」「今日禱告」。建議用在一整行短句，不要只套在半句話上。</li>
              <li>粗體：用來強調重要詞句。適合用在一句話中的重點，不建議整篇都使用粗體。</li>
              <li>引用：用來標示經文、書中摘錄或想特別保留的一段話。如果是引用聖經，建議同時寫清楚經文出處。</li>
              <li>經文：用來標示經文內容或經文段落。如果已經用「抓取經文」帶入全文，可以在札記中引用重要經文時使用。</li>
              <li>清單：用來整理多個重點，例如今天的提醒、需要禱告的事、可以實踐的行動。</li>
              <li>分隔線：用來分開不同段落，例如在「經文觀察」「我的反思」「禱告」中間加上分隔線。不要連續插入太多分隔線，以免文章變得零碎。</li>
              <li>紅字：用來提醒警醒、悔改、特別重要的地方。不要整篇都用紅字。</li>
              <li>藍字：可以用在安慰、提醒、補充說明或較柔和的重點。</li>
              <li>金字：可以用在特別珍貴的提醒、經文亮光或感恩內容。</li>
              <li>紫字：可以用在禱告、默想、敬拜或較深的反思內容。</li>
            </ul>
            <h3>小標題加顏色怎麼用</h3>
            <p>如果你想讓段落標題更清楚，可以先把那一行設成「小標題」，再替標題文字加上顏色。</p>
            <ol>
              <li>先輸入一行標題，例如「我的反思」。</li>
              <li>選取這一整行文字，或把游標放在要新增小標題的位置。</li>
              <li>先點「小標題」。</li>
              <li>接著重新選取標題文字本身。請不要把小標題前面自動加上的開頭記號一起選進去。</li>
              <li>再點想要的顏色，例如「藍字」或「金字」。</li>
              <li>點「預覽文章」或儲存草稿，確認標題大小與顏色都正確。</li>
            </ol>
            <p>提醒：小標題比較適合用在短句或段落名稱。不要一次選取太長的段落來套小標題。如果預覽時看到開頭記號沒有變成標題，可以重新選取標題文字，再依順序套用一次。</p>
            <h3>小工具搭配範例</h3>
            <ul>
              <li>小標題：經文觀察</li>
              <li>引用：「你的話是我腳前的燈，是我路上的光。」詩篇 119:105</li>
              <li>粗體：今天我被提醒，神的話不只是知識，也是帶領我走路的光。</li>
              <li>清單：今天可以操練：先安靜讀經、寫下一句觸動我的經文、用禱告回應神。</li>
              <li>分隔線：可以放在「反思」和「禱告」中間，讓文章段落更清楚。</li>
            </ul>
            <h3>草稿和正式札記差在哪裡</h3>
            <ul>
...
function setupCloudRealtime() {
  ...
  ['devotion_notes', 'devotion_prayers', 'book_projects', 'book_snapshots'].forEach((table) => {
...
function buildBackupPayload(exportedAtIso = nowIso()) {
  ...
  return {
    ...
    prayers: Array.isArray(state.prayers) ? state.prayers : [],
    drafts,
...
function renderBackupImportPreview() {
  ...
  return `
    ...
    <div class="admin-summary-grid">
      ...
      <article class="admin-stat-card">
        <span class="admin-stat-label">prayers</span>
        <strong class="admin-stat-value">${escapeHtml(String(data.prayersCount || 0))}</strong>
      </article>
      <article class="admin-stat-card">
    ...
    ${preview.result ? `
      <div class="panel compact">
        ...
        <p class="caption">新增 prayers：${escapeHtml(String(preview.result.addedPrayers || 0))} 筆｜跳過 prayers：${escapeHtml(String(preview.result.skippedPrayers || 0))} 筆</p>
    ...
    try {
      ...
      setBackupImportPreview({
        ...
        data: {
          ...
          prayersCount: Array.isArray(payload.prayers) ? payload.prayers.length : 0,
...
async function getExistingPrayerIdsForRestore() {
  const userId = getUserId();
  if (state.supabase) {
    const { data, error } = await state.supabase.from('devotion_prayers').select('id').eq('user_id', userId);
    if (error) throw error;
    return new Set((data || []).map(item => String(item.id || '')).filter(Boolean));
  }
  return new Set((state.prayers || []).map(item => String(item.id || '')).filter(Boolean));
}
...
async function captureDangerRestoreSnapshot() {
  const snapshot = {
    ...
    prayers: JSON.parse(JSON.stringify(state.prayers || [])),
...
async function clearDangerRestoreData(userId = '') {
  if (state.supabase) {
    ...
    await state.supabase.from('devotion_prayers').delete().eq('user_id', userId);
...
async function writeDangerRestorePrayers(prayers = [], userId = '') {
  const payloadPrayers = (Array.isArray(prayers) ? prayers : []).map(prayer => buildPrayerPersistencePayload({ ...prayer, user_id: userId }, userId));
  if (state.supabase && payloadPrayers.length) {
    const { error } = await state.supabase.from('devotion_prayers').insert(payloadPrayers);
    if (error) throw error;
  }
}
...
async function rollbackDangerRestore(snapshot, userId = '') {
  ...
  await writeDangerRestorePrayers(snapshot.prayers || [], userId);
...
async function requestDangerRestoreConfirmation(backup = {}) {
  ...
  const prayersCount = Array.isArray(backup.prayers) ? backup.prayers.length : 0;
  ...
  return openConfirmDialog({
    title: '確定要執行覆蓋還原嗎？',
    message: `這可能會覆蓋目前資料。請確認已經備份後再繼續。\n\n備份內容：札記 ${notesCount} 筆、禱告 ${prayersCount} 筆、選稿編排 ${draftsCount} 份、書櫃 ${libraryCount} 本。${mismatchWarning}`,
    confirmText: '確認還原',
    danger: true,
...
async function restoreBackupMerge() {
  ...
  const backupPrayers = Array.isArray(backup.prayers) ? backup.prayers : [];
  ...
  const existingPrayerIds = await getExistingPrayerIdsForRestore();
  const newPrayers = backupPrayers
    .filter(prayer => {
      const id = String(prayer?.id || '');
      const exists = !id || existingPrayerIds.has(id);
      if (exists) result.skippedPrayers += 1;
      if (!exists) existingPrayerIds.add(id);
      return !exists;
    })
    .map(prayer => buildPrayerPersistencePayload({ ...prayer, user_id: userId }, userId));
  result.addedPrayers = newPrayers.length;

  if (newPrayers.length) {
    if (state.supabase) {
      const { error } = await state.supabase.from('devotion_prayers').insert(newPrayers);
      if (error) throw error;
    } else {
      result.skippedPrayers += newPrayers.length;
      result.addedPrayers = 0;
    }
  }
  ...
  refreshUi();
  showToast(`新增 notes：${result.addedNotes}｜新增 prayers：${result.addedPrayers}｜新增 drafts：${result.addedDrafts}｜新增 library：${result.addedLibrary}`);
}
...
async function restoreBackupDanger() {
  ...
  try {
    ...
    setRestoreStatusMessage('正在還原 prayers…');
    await writeDangerRestorePrayers(backup.prayers || [], userId);
...
function getAuthInlineSubmitLabel() {
  if (state.authInlineMode === 'password-recovery') return '更新密碼';
...
async function submitAuthInlineForm(event = null) {
  event?.preventDefault();
  if (state.authInlineSubmitting) return;
  ...
  state.authInlineSubmitting = true;
  syncAuthInlineSubmitButton(true);
  try {
    await action();
  } finally {
...
function bindEvents() {
  ...
  if (els.authInlineForm) {
    els.authInlineForm.addEventListener('submit', event => submitAuthInlineForm(event).catch(handleError));
  } else {
  ...
  els.prayerDetailPanel?.addEventListener('click', (event) => {
    ...
    if (openNoteButton) {
      openNoteReaderNote(openNoteButton.dataset.prayerOpenNote || '');
    }
  });
  ...
  els.noteReaderSearch?.addEventListener('input', event => {
    ...
    syncNoteReaderControls();
  });
  els.noteReaderSearch?.addEventListener('keydown', event => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    openNoteReaderSearchModal();
  });
  els.noteReaderSearchScope?.addEventListener('change', event => {
    state.noteReaderSearchScope = String(event.target.value || 'all');
    syncNoteReaderControls();
  });
  els.noteReaderClearSearch?.addEventListener('click', () => {
    state.noteReaderSearch = '';
    syncNoteReaderControls();
    if (!els.noteReaderSearchModal?.classList.contains('hidden')) renderNoteReaderSearchResults();
    els.noteReaderSearch?.focus();
  });
  els.noteReaderSearchBtn?.addEventListener('click', () => openNoteReaderSearchModal());
  els.noteReaderSearchModalClose?.addEventListener('click', closeNoteReaderSearchModal);
  els.noteReaderSearchModalCloseBottom?.addEventListener('click', closeNoteReaderSearchModal);
  els.noteReaderSearchBackdrop?.addEventListener('click', closeNoteReaderSearchModal);
  els.noteReaderSearchModalClear?.addEventListener('click', () => {
    resetNoteReaderFilters();
    renderNoteReader();
    renderNoteReaderSearchResults();
    els.noteReaderSearch?.focus();
  });
  ...
  els.noteReaderReadingEdit?.addEventListener('click', () => {
    const noteId = state.noteReaderReadingId;
    closeNoteReaderReadingModal();
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && !els.noteReaderReadingModal?.classList.contains('hidden')) closeNoteReaderReadingModal();
    if (event.key === 'Escape' && !els.noteReaderSearchModal?.classList.contains('hidden')) closeNoteReaderSearchModal();
...
async function loadAllData({ silent = false, syncReason = '', reason = '', minIntervalMs = 0, force = false } = {}) {
  ...
  const loadTask = (async () => {
    ...
    if (state.supabase && state.currentUser) {
      ...
      if (!silent) refreshUi();
      const [notesRes, prayersRes, booksRes, snapshotsRes] = await Promise.all([
        state.supabase.from('devotion_notes').select('*').eq('user_id', userId).order('updated_at', { ascending: false }),
        state.supabase.from('devotion_prayers').select(PRAYER_SELECT).eq('user_id', userId).order('updated_at', { ascending: false }),
      ...
      egressDebugLog('loadAllData:tables', {
        ...
        prayers: { count: (prayersRes.data || []).length, bytes: estimatePayloadBytes(prayersRes.data || []) },
...
function getBookDraftSettingsElements() {
  return {
    ...
    intro: document.getElementById('book-draft-settings-intro'),
    ...
    subtitle: document.getElementById('book-draft-settings-subtitle'),
    author: document.getElementById('book-draft-settings-author'),
...
function renderBookDraftSettingsCoverPreview(coverDataUrl = '', fallbackTitle = '目前封面') {
  const { coverPreview } = getBookDraftSettingsElements();
  if (!coverPreview) return;
  if (coverDataUrl) {
    coverPreview.innerHTML = `
      <img src="${escapeHtml(coverDataUrl)}" alt="${escapeHtml(fallbackTitle)}封面預覽" />
      <div class="book-export-settings-cover-copy">
        <strong>目前封面</strong>
        <span>重新選擇圖片後，會更新這本書的封面資料。</span>
      </div>
    `;
  ...
  coverPreview.innerHTML = `
    <div class="book-export-settings-cover-copy">
      ...
      <span>可先不放封面，也可以在這裡補上適合這本書的圖片。</span>
    </div>
  `;
...
function populateBookDraftSettingsModal(book) {
  ...
  modalEls.title.textContent = `編輯書籍資料：${getBookDraftLabel(book)}`;
  if (modalEls.intro) {
    modalEls.intro.textContent = '整理這本書的書名、作者、簡介與封面，匯出 EPUB 時會使用這些資料。';
  }
  ...
  renderBookDraftSettingsCoverPreview(book.cover_data_url || '', getBookDraftLabel(book));
}

async function openBookDraftSettingsModal(bookId = '') {
...
function syncBookDraftSettingsActionState(isSaving = false) {
  ...
  saveBtn.textContent = isSaving ? '儲存中...' : '儲存書籍資料';
}
...
async function saveBookDraftSettings() {
  ...
  const book = await loadBookProjectDetail(bookId) || state.books.find(item => item.id === bookId);
  if (!book) throw new Error('找不到要更新的書籍資料。');
...
function ensureBookDraftSettingsUi() {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-card book-draft-settings-card" role="dialog" aria-modal="true" aria-labelledby="book-draft-settings-title">
      <div class="panel-header book-draft-settings-header">
        <div>
          <h2 id="book-draft-settings-title">編輯書籍資料</h2>
          <p id="book-draft-settings-intro" class="modal-intro">整理這本書的書名、作者、簡介與封面，匯出 EPUB 時會使用這些資料。</p>
        </div>
      ...
      <form id="book-draft-settings-form" class="book-draft-settings-form">
        <div id="book-draft-settings-body" class="book-draft-settings-scroll">
          <div class="book-draft-settings-grid">
            <label>書名
              <input id="book-draft-settings-draft-title" required placeholder="例：五月靈修選集" />
            </label>
            <label>副標
              <input id="book-draft-settings-subtitle" placeholder="例：安靜同行的三十天" />
            </label>
            <label>作者
              <input id="book-draft-settings-author" placeholder="例：Jeremiah" />
            </label>
            <label>模板
              <select id="book-draft-settings-template">
                <option value="devotion">靈修札記版</option>
                <option value="sermon">講章整理版</option>
                <option value="testimony">見證合集版</option>
              </select>
            </label>
            <label class="book-draft-settings-span-2">書籍簡介
              <textarea id="book-draft-settings-description" rows="4" placeholder="介紹這本書的主題、收錄範圍與閱讀方向"></textarea>
            </label>
            <div class="book-draft-settings-span-2 book-export-settings-cover-block">
              <div id="book-draft-settings-cover-preview" class="book-export-settings-cover-preview is-empty"></div>
              <label>封面圖片
        ...
        <div class="book-draft-settings-actions" role="group" aria-label="編輯書籍資料操作">
          <button id="cancel-book-draft-settings-btn" class="ghost-btn" type="button">取消</button>
          <button id="save-book-draft-settings-btn" class="secondary-btn" type="submit">儲存書籍資料</button>
        </div>
      </form>
    </div>
  `;
  ...
  modalEls.cover?.addEventListener('change', async event => {
    ...
    const fallbackTitle = modalEls.draftTitle?.value?.trim() || '目前封面';
    if (!file) {
      renderBookDraftSettingsCoverPreview(modalEls.form?.dataset.coverDataUrl || '', fallbackTitle);
      return;
    }
    const dataUrl = await fileToDataUrl(file);
    renderBookDraftSettingsCoverPreview(dataUrl, fallbackTitle);
  });
}
...
function populateBookExportSettingsModal(book) {
  ...
  modalEls.intro.textContent = `最後確認「${getBookDraftLabel(book)}」的書籍資料與輸出狀態；章節順序請先在編排區儲存。`;
    ...
    <div class="modal-card book-export-settings-card" role="dialog" aria-modal="true" aria-labelledby="book-export-settings-title">
      <div class="panel-header book-export-settings-header">
        <div>
          ...
          <p id="book-export-settings-intro" class="modal-intro">最後確認目前書稿的書籍資料與輸出狀態；章節順序請先在編排區儲存。</p>
        </div>
...
function getVisiblePrayerRecords() {
  const filter = String(state.prayerFilter || 'all');
  const records = sortPrayerRecords(state.prayers || []);
  if (filter === 'all') return records;
  return records.filter(prayer => normalizePrayerStatus(prayer.status) === filter);
}

function setPrayerFormFeedback(message = '', isError = false) {
  if (!els.prayerFormFeedback) return;
  const text = String(message || '').trim();
  els.prayerFormFeedback.textContent = text;
  els.prayerFormFeedback.classList.toggle('hidden', !text);
  els.prayerFormFeedback.classList.toggle('is-error', !!isError);
}

function renderPrayerRelatedNoteOptions(selectedId = els.prayerRelatedNoteSelect?.value || '') {
  if (!els.prayerRelatedNoteSelect) return;
  const publishedNotes = getPublishedNotes();
  const selected = String(selectedId || '');
  els.prayerRelatedNoteSelect.innerHTML = [
    '<option value="">不關聯札記</option>',
    ...publishedNotes.map(note => `
      <option value="${escapeHtml(String(note.id || ''))}" ${String(note.id || '') === selected ? 'selected' : ''}>
        ${escapeHtml(getNoteDisplayTitle(note))}
      </option>
    `),
  ].join('');
}
...
function clearPrayerForm() {
  if (els.prayerId) els.prayerId.value = '';
  if (els.prayerTitleInput) els.prayerTitleInput.value = '';
  if (els.prayerRequestInput) els.prayerRequestInput.value = '';
  if (els.prayerStatusSelect) els.prayerStatusSelect.value = PRAYER_STATUS_ACTIVE;
  if (els.prayerRelatedNoteSelect) els.prayerRelatedNoteSelect.value = '';
  if (els.prayerReviewInput) els.prayerReviewInput.value = '';
  if (els.prayerGraceReflectionInput) els.prayerGraceReflectionInput.value = '';
...
function openPrayerForm(prayer = null, { focus = true } = {}) {
  ...
  if (els.prayerStatusSelect) els.prayerStatusSelect.value = normalizePrayerStatus(prayer?.status || PRAYER_STATUS_ACTIVE);
  if (els.prayerRelatedNoteSelect) els.prayerRelatedNoteSelect.value = prayer?.related_note_id || '';
  if (els.prayerReviewInput) els.prayerReviewInput.value = prayer?.review_text || '';
  if (els.prayerGraceReflectionInput) els.prayerGraceReflectionInput.value = prayer?.grace_reflection || '';
  setPrayerFormFeedback('');
  els.prayerFormPanel?.scrollIntoView({ behavior: 'smooth', block: 'start' });
...
function getPrayerFormRelatedNoteId() {
  const selectedId = String(els.prayerRelatedNoteSelect?.value || '');
  if (!selectedId) return '';
  return getPublishedNotes().some(note => String(note.id || '') === selectedId) ? selectedId : '';
}

async function savePrayer(event) {
  event?.preventDefault?.();
  if (state.prayerSaving) return;
  if (!isPrayerCloudReady()) {
    setPrayerFormFeedback('禱告紀錄需要登入雲端帳號後才能同步。', true);
    return;
  }
  ...
  const shouldMarkReviewed = [PRAYER_STATUS_REVIEWING, PRAYER_STATUS_ANSWERED].includes(status)
    ...
    && (
      ...
      || reviewText !== String(existing?.review_text || '')
      || graceReflection !== String(existing?.grace_reflection || '')
    );
  const payload = buildPrayerPersistencePayload({
    id,
    user_id: getUserId(),
    title,
    prayer_request: prayerRequest,
    status,
    related_note_id: getPrayerFormRelatedNoteId(),
    review_text: reviewText,
    grace_reflection: graceReflection,
    last_reviewed_at: shouldMarkReviewed ? now : (existing?.last_reviewed_at || ''),
    created_at: existing?.created_at || now,
    updated_at: now,
  }, getUserId());

  state.prayerSaving = true;
  els.prayerSaveButton?.toggleAttribute('disabled', true);
  setPrayerFormFeedback('正在儲存禱告紀錄…');
  try {
    const query = id
      ? state.supabase.from('devotion_prayers').update(payload).eq('id', id).eq('user_id', getUserId()).select(PRAYER_SELECT).single()
      : state.supabase.from('devotion_prayers').insert(payload).select(PRAYER_SELECT).single();
    const { data, error } = await query;
    if (error) throw error;
    const saved = normalizePrayerRecord(data || payload, getUserId());
    state.prayers = sortPrayerRecords([saved, ...state.prayers.filter(prayer => String(prayer.id || '') !== String(saved.id || ''))]);
    markCloudSynced(id ? '禱告紀錄已更新並同步。' : '禱告紀錄已新增並同步。');
    closePrayerForm();
    renderPrayerWorkspace();
    ...
    state.prayerSaving = false;
    els.prayerSaveButton?.toggleAttribute('disabled', false);
  }
}
  ...
  return `
    <article class="prayer-card" data-prayer-id="${escapeHtml(String(prayer.id || ''))}" data-testid="prayer-card">
      ...
      <div class="prayer-card-actions">
        ...
        <button class="secondary-btn small" type="button" data-review-prayer="${escapeHtml(String(prayer.id || ''))}" data-testid="prayer-review-button">回顧</button>
      </div>
    </article>
  `;
}

function renderPrayerWorkspace() {
  ...
  els.prayerList.innerHTML = visiblePrayers.map(renderPrayerCard).join('');
}
...
function getBookDraftWorkflowState(book) {
  ...
  let nextAction = '確認書籍資料後匯出 EPUB。';
  if (!hasBookInfo) {
    currentKey = 'book-info';
    currentLabel = '書籍資料未完成';
    nextAction = '先填寫書名，建立這份書稿。';
  } else if (!hasChapters) {
    ...
    currentKey = 'check';
    currentLabel = '建議補齊';
    nextAction = `${buildPublishingAffectedText(firstSuggestion, { maxTitles: 2 })}也可以先匯出 EPUB。`;
  }
...
function ensureBookDraftWorkspaceUi() {
  ...
  if (titleLabel) {
    titleLabel.childNodes[0].textContent = '書名';
    els.bookTitle.placeholder = '例：五月靈修選集';
  }
  const descriptionLabel = els.bookDescription?.closest('label');
  if (descriptionLabel) {
    descriptionLabel.childNodes[0].textContent = '書籍簡介';
    els.bookDescription.placeholder = '記下這本書的主題、收錄範圍與閱讀方向';
  }
  ...
  if (els.selectedBookPanel && els.bookCoverPreview && tocPreviewPanel && els.chaptersList && actionRow && chapterHeading) {
    ...
    if (!rightFooter) {
      ...
      rightFooter.className = 'book-draft-modal-right-footer book-draft-workspace-toolbar';
    }
    rightFooter.classList.add('book-draft-workspace-toolbar');
    let toolbarActions = document.getElementById('book-draft-toolbar-actions');
    if (!toolbarActions) {
      toolbarActions = document.createElement('div');
      toolbarActions.id = 'book-draft-toolbar-actions';
    ...
    if (arrangementStatus) rightFooter.insertBefore(arrangementStatus, toolbarActions);
    ...
    exportSettingsBtn.textContent = '匯出 EPUB';
    toolbarActions.appendChild(exportSettingsBtn);
    ...
    toolbarActions.appendChild(modalStartBtn);
    const saveBtn = document.getElementById('save-book-arrangement-btn');
    if (saveBtn) toolbarActions.prepend(saveBtn);
    document.getElementById('book-draft-modal-mobile-actions')?.remove();
    document.getElementById('save-book-arrangement-mobile-btn')?.remove();
    document.getElementById('open-book-export-settings-mobile-btn')?.remove();
    document.getElementById('modal-start-current-book-mobile-btn')?.remove();
...
function getNoteReaderDateValue(note, field = 'updated') {
  const raw = field === 'created'
    ? (note?.created_at || note?.createdAt || note?.updated_at || note?.updatedAt || '')
...
function normalizeNoteReaderSearchScope(scope = 'all') {
  const value = String(scope || 'all').trim();
  return ['all', 'title', 'scripture', 'content', 'category', 'tags'].includes(value) ? value : 'all';
}

function getNoteReaderSearchHaystack(note, scope = state.noteReaderSearchScope) {
  const normalizedScope = normalizeNoteReaderSearchScope(scope);
  const fields = {
    title: [note.title],
  ...
  const values = normalizedScope === 'all'
    ? [...fields.title, ...fields.scripture, ...fields.content, ...fields.category, ...fields.tags]
    : fields[normalizedScope];
...
function compareNotesForReading(left, right) {
  ...
  return compareNotesByRecentUpdate(left, right);
}

function compareNotesByRecentUpdate(left, right) {
  const updatedDelta = getNoteReaderDateValue(right, 'updated') - getNoteReaderDateValue(left, 'updated');
  if (updatedDelta) return updatedDelta;
  const createdDelta = getNoteReaderDateValue(right, 'created') - getNoteReaderDateValue(left, 'created');
  if (createdDelta) return createdDelta;
  return getNoteDisplayTitle(left).localeCompare(getNoteDisplayTitle(right), 'zh-Hant');
}

function getNoteReaderRawDate(note = {}, field = 'updated') {
  return field === 'created'
    ? (note.created_at || note.createdAt || note.updated_at || note.updatedAt || '')
    : (note.updated_at || note.updatedAt || note.created_at || note.createdAt || '');
}

function getNoteReaderUpdatedDate(note = {}) {
  return getNoteReaderRawDate(note, 'updated');
}

function getNoteReaderCreatedDate(note = {}) {
  return getNoteReaderRawDate(note, 'created');
}
...
function getRecentEditedNotes(limit = 5) {
  return getPublishedNotes()
    .slice()
...
function resetNoteReaderFilters() {
  state.noteReaderSearch = '';
  state.noteReaderSearchScope = 'all';
...
function hasActiveNoteReaderFilters() {
  return !!state.noteReaderSearch
    || state.noteReaderSearchScope !== 'all'
    || !!state.noteReaderCategory
    || !!state.noteReaderTag
    || state.noteReaderSort !== 'updated-desc';
...
function syncNoteReaderControls() {
  ...
  state.noteReaderSearchScope = normalizeNoteReaderSearchScope(state.noteReaderSearchScope);
  ...
  if (els.noteReaderSearchScope) els.noteReaderSearchScope.value = state.noteReaderSearchScope;
  ...
  if (els.noteReaderResultCount) {
    ...
    els.noteReaderResultCount.textContent = `共 ${totalCount} 篇正式札記`;
  }
  els.noteReaderClearSearch?.toggleAttribute('disabled', !state.noteReaderSearch.trim());
  els.noteReaderResetFilters?.toggleAttribute('disabled', !hasActiveNoteReaderFilters());
}

function syncNoteReaderModalLock() {
...
function scrollNoteReaderIntoView() {
  ...
  requestAnimationFrame(() => {
    document.getElementById('view-note-reader')?.scrollIntoView({ block: 'start', behavior: 'smooth' });
  });
}

function getNoteReaderMetaItems(note, { includeScripture = true, includeCreated = false } = {}) {
  const tags = getNoteTagList(note);
  const items = [
    includeScripture ? `經文｜${sanitizeDisplayText(note.scripture_reference, '未填經文')}` : '',
    `分類｜${sanitizeDisplayText(note.category, '未分類')}`,
    `標籤｜${tags.length ? tags.map(tag => `#${tag}`).join(' ') : '未設標籤'}`,
    includeCreated ? `日期｜${formatDate(getNoteReaderCreatedDate(note))}` : '',
    `最近更新｜${formatDate(getNoteReaderUpdatedDate(note))}`,
  ];
...
function renderNoteReaderEmptyState() {
  if (!els.noteReaderList) return;
  els.noteReaderList.className = 'note-reader-recent-list note-reader-empty-state empty-state';
  els.noteReaderList.innerHTML = `
    <strong>目前還沒有札記</strong>
    <p>你可以先到「寫札記」記下今天的靈修內容，之後會在這裡看到最近編輯的札記。</p>
...
function renderNoteReaderNoSearchResultsState() {
  if (!els.noteReaderSearchResults) return;
  els.noteReaderSearchResults.className = 'note-reader-search-results note-reader-empty-state empty-state';
  els.noteReaderSearchResults.innerHTML = `
    <strong>找不到符合條件的札記</strong>
    <p>你可以調整關鍵字、分類或標籤後再試一次。</p>
    <div class="note-reader-empty-actions">
      <button class="ghost-btn" type="button" data-note-reader-reset-empty>清除搜尋</button>
    </div>
  `;
}

function renderNoteReaderSearchResults() {
  if (!els.noteReaderSearchResults) return;
  const notes = getNotesForReading();
  if (!notes.length) {
    renderNoteReaderNoSearchResultsState();
    return;
  }
  els.noteReaderSearchResults.className = 'note-reader-search-results';
  els.noteReaderSearchResults.innerHTML = notes
    .map(note => renderNoteReaderCard(note, { testId: 'note-reader-search-result' }))
    .join('');
}
...
function renderNoteReaderReadingContent(note) {
  ...
  els.noteReaderReadingContent.innerHTML = `
    <header class="note-preview-header note-reader-modal-title-block">
      <h2 id="note-reader-reading-title">${escapeHtml(title)}</h2>
      ${renderNoteReaderMeta(note, { includeCreated: true })}
    </header>
    ${visibleSummary ? `<section class="note-preview-summary">
...
function getPublishedNoteById(noteId = '') {
  const id = String(noteId || '');
  return getPublishedNotes().find(note => String(note.id || '') === id) || null;
}

function openNoteReaderSearchModal() {
  syncNoteReaderControls();
  renderNoteReaderSearchResults();
  closeNoteReaderReadingModal();
  els.noteReaderSearchModal?.classList.remove('hidden');
  els.noteReaderSearchModal?.setAttribute('aria-hidden', 'false');
  syncNoteReaderModalLock();
  requestAnimationFrame(() => els.noteReaderSearchModalClose?.focus());
}

function closeNoteReaderSearchModal() {
  els.noteReaderSearchModal?.classList.add('hidden');
  els.noteReaderSearchModal?.setAttribute('aria-hidden', 'true');
  syncNoteReaderModalLock();
}

function openNoteReaderReadingModal(note) {
  if (!note) return;
  state.noteReaderReadingId = String(note.id || '');
  renderNoteReaderReadingContent(note);
  closeNoteReaderSearchModal();
  els.noteReaderReadingModal?.classList.remove('hidden');
  els.noteReaderReadingModal?.setAttribute('aria-hidden', 'false');
  syncNoteReaderModalLock();
  requestAnimationFrame(() => els.noteReaderReadingClose?.focus());
}

function closeNoteReaderReadingModal() {
  state.noteReaderReadingId = '';
  els.noteReaderReadingModal?.classList.add('hidden');
  els.noteReaderReadingModal?.setAttribute('aria-hidden', 'true');
  syncNoteReaderModalLock();
}
...
function renderNoteReader() {
  ...
  const recentNotes = getRecentEditedNotes(5);
  els.noteReaderList.className = 'note-reader-recent-list';
  els.noteReaderList.innerHTML = recentNotes.map(note => renderNoteReaderCard(note)).join('');
  bindNoteReaderActions();
}
...
function openNoteReader() {
  ...
  setView('note-reader');
  scrollNoteReaderIntoView();
}

function openNoteReaderNote(noteId) {
  const note = getPublishedNoteById(noteId);
  if (document.body.dataset.currentView !== 'note-reader') setView('note-reader');
  if (!note) {
    showToast('找不到這篇正式札記。');
    return;
  }
  openNoteReaderReadingModal(note);
}
...
function renderBookDraftCard(book, { current = false } = {}) {
  ...
  const statusLabel = current ? '正在編排' : '待整理';
  ...
  return `
    <article class="card book-draft-card ${current ? 'is-current-draft selected' : ''}" data-book-draft-card="${book.id}" data-testid="book-draft-card">
      ...
      <div class="card-actions book-draft-actions">
        ${current
          ? `
            <button class="secondary-btn" data-open-book-draft="${book.id}" data-testid="book-open-draft">繼續編排</button>
            <button class="ghost-btn" data-go-content-library="${book.id}" data-testid="book-go-content-library">加入札記</button>
            <button class="ghost-btn" data-open-selection-settings="${book.id}" data-testid="book-open-selection-settings">書籍資料</button>
            <button class="ghost-btn" data-open-export-settings="${book.id}" data-testid="book-open-export-settings">匯出 EPUB</button>
          `
          : `
...
function bindBookDraftCardActions(scope) {
  ...
  scope.querySelectorAll('[data-open-selection-settings]').forEach(btn => btn.addEventListener('click', () => openBookDraftSettingsModal(btn.dataset.openSelectionSettings).catch(handleError)));
...
async function saveBook() {
  ...
  if (els.bookSaveFeedback) {
    els.bookSaveFeedback.textContent = isNewDraft
      ? (previousSelectedBookId ? `已建立書稿：${payload.title}` : `已建立並開始編排：${payload.title}`)
      : '書籍資料已儲存';
...
function injectReaderViewStyles() {
  ...
  style.textContent = `
    ...
    #view-reader .reader-close-button { position: absolute; top: 14px; right: clamp(12px, 3vw, 28px); z-index: 30; min-width: 64px; min-height: 44px; padding: 0 16px; border: 1px solid rgba(80,70,55,.16); border-radius: 999px; background: rgba(255,255,255,.86); color: #26494c; font-weight: 700; box-shadow: 0 10px 28px rgba(45,35,25,.12); backdrop-filter: blur(12px); pointer-events: auto; touch-action: manipulation; transition: opacity .18s ease, transform .18s ease; }
    ...
    #view-reader.reader-panel-open .reader-close-button { opacity: .96; pointer-events: auto; transform: none; }
    ...
    @media (max-width: 1023px) {
      ...
      #view-reader .reader-close-button { top: 10px; right: 10px; min-width: 64px; min-height: 44px; padding-inline: 14px; font-size: .82rem; }
